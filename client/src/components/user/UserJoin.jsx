...
const UserJoin = () => {
  ...
  const NameCheckFunc = (e) => {
    ...
    if (!lengthNickname(youName)) {
      setNameInfo(
        <span className="nameInfo invalid">
          닉네임은 한글 2-5자로 입력해주세요!
        </span>
      );
      return;
    }
    ...
    axios.post("/api/user/namecheck", body).then((response) => {
      if (response.data.success) {
        if (response.data.check) {
          setNameCheck(true);
          setNameInfo(
            <span className="nameInfo">사용 가능한 닉네임입니다.</span>
          );
        } else {
          setNameInfo(
            <span className="nameInfo invalid">
      ...
      <form action="/" className="join__form">
        <fieldset>
          ...
          <div className="join__inputInner">
            ...
            />
            <button className="check__btn" onClick={(e) => NameCheckFunc(e)}>
              중복검사
            </button>
          ...
          <div className="join__inputInner">
            ...
            <input
              ...
              onBlur={async (e) => {
                ...
                if (isValid) {
                  // 서버에 이메일 중복 체크 요청
                  const response = await axios.post("/api/user/emailcheck", {
                    email: e.currentTarget.value,
                  });
                  if (!response.data.check) {
                    // check는 서버에서 반환하는 필드로, 이메일이 존재하면 true, 존재하지 않으면 false
                    setEmailInfo(
                      <span className="nameInfo invalid">
                        이미 가입된 이메일입니다
                      </span>
          ...
          <div className="join__inputInner">
            <label htmlFor="youPassC" className="join__label required">
              비밀번호 확인<em>*</em>
            </label>
            <input
              type="text"
